;(function() {
    ...
    class Slim extends HTMLElement {
        ...
        _initInteractiveEvents() {
            if (!this.__eventsInitialized && (Slim.autoAttachInteractionEvents || this.isInteractive || this.hasAttribute('interactive'))) Slim._nativeEvents.forEach(eventType => {
        ...
        _captureBindings() {
            ...
            for (let child of allChildren) {
                ...
                if (child.attributes) for (let i = 0; i < child.attributes.length; i++) {
                    const isNativeEventAttribute = Slim._nativeEvents.indexOf(child.attributes[i].nodeName >= 0) || Slim._nativeEvents.some(nativeEventName => 'on' + nativeEventName === child.attributes[i].nodeName)
